...
echoheader("<em class=\"icon-credit-card\"></em> " . $lang['hypayment_t'], $lang['hypayment_d']);

echo "<div class=\"box\"><div class=\"box-header\"><div class=\"title\">{$lang['success_transactions']}</div></div><div class=\"box-content\">";

$db->query("DELETE FROM " . PREFIX . "_hypayments WHERE date < " . ($_TIME - 21600) . " AND gateway = 'jahanpay' AND verified = '0'");
...
<td>{$lang['email']}</td><td>{$lang['info']}</td><td>{$lang['amount']}</td><td>{$lang['date']}</td>
<td>{$lang['situation']}</td><td>{$lang['actions']}</td></tr></thead><tbody>
HTML;

$db->query("SELECT * FROM ".PREFIX."_hypayments WHERE verified = '1' AND gateway = 'jahanpay' ORDER BY id DESC LIMIT $startpoint, $endpoint");
if ($db->num_rows()) {
...
<tr>
    <td>{$startpoint}</td>
    <td>{$row['transid']}</td>
    <td>{$row['hypay_name']}</td>
    <td>{$row['hypay_email']}</td>
    <td style="padding:3px 2px 0 2px"><button onclick="DLEalert('{$row['hypay_info']}', '{$lang['info']}'); return false;" class="btn btn-xs btn-primary" style="width:100%;padding:2px 0 1px;"><em class="icon-pencil"></em></button></td>
    <td>{$row['hypay_price']} {$lang['toman']}</td>
    <td>{$row['date']}</td>
    <td>{$row['verified']}</td>
    <td style="padding:3px 2px 0 2px"><a href="?mod=hypayment&action=delete&id={$row['id']}" style="padding:2px 6px;width:100%;" class="btn btn-danger"><em class="icon-trash"></em></a></td>
</tr>
HTML;
    }
}
...
$pages = "";
$row = $db->super_query("SELECT COUNT(*) AS counter FROM ".PREFIX."_hypayments WHERE gateway = 'jahanpay' AND verified = '1'");
$row = $row['counter'];
